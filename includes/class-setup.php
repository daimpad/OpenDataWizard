...
class ODW_Setup {
    ...
    public static function init(): void {
        add_action( 'admin_init',   [ self::class, 'maybe_create_demo' ] );
        add_action( 'admin_init',   [ self::class, 'handle_dismiss' ] );
        add_action( 'admin_notices', [ self::class, 'render_welcome_notice' ] );
    }
    ...
    private static function create_demo_dataset(): int {
        ...
        update_post_meta( $post_id, '_odw_description', __(
        ...
        if ( $file_id ) {
            ...
            if ( function_exists( 'carbon_set_post_meta' ) ) {
                ...
                if ( $file_url ) {
                    carbon_set_post_meta( $post_id, 'odw_distributions', [
                        [
                            'access_url' => $file_url,
                            'format'     => 'CSV',
                            'byte_size'  => '',
    ...
    public static function handle_dismiss(): void {
        ...
        wp_safe_redirect( remove_query_arg( [ 'odw_dismiss_welcome', '_wpnonce' ] ) );
        exit;
    }
