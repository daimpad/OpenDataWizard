...
class ODW_Shortcode {
	...
	private static function collect_metadata_rows( int $post_id, string $license_label, string $file_size ): array {
		...
		// Engagementfelder sind seit v2.41.0 eine Mehrfachauswahl — alle anzeigen.
		$add(
			__( 'Engagementfeld', 'open-data-wizard' ),
			class_exists( 'ODW_Fields' ) ? implode( ', ', ODW_Fields::vocabulary_labels( $post_id, 'odw_engagementfeld', 'engagementfeld' ) ) : ''
		);
